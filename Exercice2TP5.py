...
def definir(fen,entrerCH):    
    ...
    if ch=='':
        ...
        if (len(ch)<=50):
            Resultat.config(text="Résultat",command=lambda:crypter(ch))
            Resultat.place(x=380,y=270)
        else:
            entrerCH.delete(0, END)
            messagebox.showerror("Error","La taille de la chaine doit etre <=50 !",parent=fen)
                
    else:  
        entrerCH.delete(0, END)
        messagebox.showerror("Error"," La chaine ch doit etre formée uniquement\npar des lettres alphabétiques !",parent=fen)
...
def ex1():
    ...
    stroke_text(430, 80, "Ce programme permet de calculer le nombre d’occurrences successives\nde chaque caractére d'une chaine saisie.",'white', 'green')
